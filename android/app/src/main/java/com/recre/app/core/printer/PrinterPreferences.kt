...
class PrinterPreferences @Inject constructor(
    ...
    /**
     * Id del modelo/perfil persistido (T-105). `null` si el técnico
     * nunca eligió uno — en ese caso el resto del subsistema asume
     * [PrinterProfiles.POR_DEFECTO] (PT210) para no romper T-62.
     */
    val perfilIdFlow: Flow<String?> = dataStore.data.map { prefs -> prefs[KEY_PERFIL] }

    /** Snapshot puntual del id de perfil persistido. */
    suspend fun perfilId(): String? = perfilIdFlow.first()

    suspend fun setPerfil(perfil: PrinterProfile?) {
        dataStore.edit { prefs ->
            if (perfil == null) {
                prefs.remove(KEY_PERFIL)
            } else {
                prefs[KEY_PERFIL] = perfil.id.name
            }
        }
    }
    ...
    private companion object {
        ...
        val KEY_PERFIL = stringPreferencesKey("impresora_perfil")
    }
}
