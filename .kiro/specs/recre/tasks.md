...
- [x] **T-241** Skeletons de carga, pull-to-refresh (Android) y swipe actions en listados. — **Hecho.** (1) skeletons de carga en TODOS los listados (gestión Máquinas/Locales/Instalaciones/Averías + Deudas gestor/local) vía `ListSkeleton`; (2) `Modifier.animateItem()` en los listados de entidad con el muelle espacial de `RecreMotion` (T-230); (3) pull-to-refresh ya existía en los listados de técnico. **Swipe actions: DESCARTADO por decisión de producto** (2026-06-16) — las cards no lo necesitan; no se añade `SwipeToDismissBox`.
- [x] **T-242** Wizards multipaso (alta de instalación, alta de máquina) con progreso y validación por paso. — **Hecho.** Ambos altas son wizards de 2 pasos con `StepIndicator` (T-229) y "Siguiente" gateado por validez del paso: **instalación** (Qué se instala → Condiciones) y **máquina** (Identificación → Datos económicos). El wizard solo aplica al **alta**; edición y cierre siguen en form único. Reconstrucción verificada por compilación + diff de comportamiento (navegación/loader/errores idénticos). Validado: compileDebugKotlin + testDebugUnitTest 93/0 + lintDebug 0 errores.
- [x] **T-243** Drawers y toasts (`sonner` web / equivalente Compose) consistentes con los tokens y el motion. — **Hecho (capa de feedback transitorio, A-SNACKBAR-COLA).** **Web:** Toaster (sonner) tokenizado global → icono de rol siempre (success/warning/danger/info + spinner info de loading), surface-1 + borde + sombra + radio 12px y shake danger sutil en error; helper `toastEdge()` (pending→success/error, error sticky, refuerza SSOT); `ChipColaOffline` NEUTRO. El primitivo de drawer (`sheet.tsx`) ya cumplía tokens+motion (surface-1, ease-standard, focus ring 2px) → sin cambios. **Android:** `RecreSnackbarHost` (drop-in tokenizado) + `RecreSnackbarVisuals`/`mostrar()`; swap del host en las 15 pantallas con snackbar; las 8 de gestión muestran su error con icono danger. El chip de cola offline ya existía como `OfflineBadge` (no se duplica). Validado web (tsc+lint+vitest 65/0+prettier) y android (compile+lint+test, verde). **FALTA (incremental, no bloqueante):** (1) adoptar `mostrar(estado, …)` en las 7 pantallas no-gestión (averías/deudas llevan estados info/neutro, no se reclasifican a error a ciegas); (2) iconos Phosphor (hoy Material Icons Extended; Phosphor no es dependencia → swap cosmético diferido a T-258); (3) la **migración completa de formularios a drawer/sheet** (`C-DRAWER-SHEET-01`, alta/edición de inventario sin ruta nueva) es trabajo por-entidad aparte de esta capa de feedback.
- [x] **T-244** Transiciones de elemento compartido (shared-bounds/element): Compose en Android; en web vía `motion` (layout), realzable con View Transitions nativas cuando entre **T-256** (React 19). — **Hecho (infraestructura Android + 1 par representativo).** `SharedTransition.kt` aísla el opt-in experimental: `LocalSharedTransitionScope` (lo provee el `SharedTransitionLayout` que envuelve el NavHost) + `LocalNavAnimatedVisibilityScope` (lo provee cada `composable`) + `Modifier.recreSharedBounds(key)` con **fallback** (sin scopes → no-op; reduced-motion → `snap`). Pares aplicados (Android): nombre del local en `LocalCard` ↔ título de `LocalDetalleScreen`, y `DeudasGestorScreen` ↔ `DeudasLocalScreen` (mismo nombre de local); claves `local-nombre-$id` / `deuda-nombre-$id`. Validado compile+lint+test (verde). **FALTA (incremental / fuera de alcance):** (1) **web**: la transición nativa lista→detalle cross-route necesita la **View Transitions API** (React 19/Next 16) → **diferida a T-256** como dice la propia tarea; sin candidato within-page claro, no se añade `motion layout` especulativo ahora. (2) Las listas de **gestión** (máquinas/instalaciones/licencias) abren un **formulario de edición** con título genérico ("Editar X"), sin un texto-nombre que casar → ahí el morph de nombre no aplica; quedan como adopción opcional sólo si se les da una cabecera con el nombre de la entidad. (3) **`sharedElement` de imagen N/A**: el modelo de máquina/local no tiene foto; cuando exista, añadir la foto como `sharedElement`. (4) **Sin verificación visual** (no ejecuto la app); requiere QA a ojo del morph lista→detalle.

### F6 — Accesibilidad

- [x] **T-245** Barrido WCAG final (European Accessibility Act): AA 4.5:1 obligatorio en **ambos modos**, estado nunca solo-color, foco visible, `aria-live` que anuncia cambios (no el estado inicial), targets táctiles. — **Hecho (barrido de código).** **Web:** (1) **foco visible** — átomos base (button/input/select/textarea) y comunes (collapsible/combobox/field/search-field/date-field) suben de `ring-1` (1px) al normativo `ring-2` + `ring-offset-2` (SC 2.4.13, en toda la app a la vez; eran defaults de shadcn). (2) **estado no solo por color** — 7 hardcodes `amber/emerald` → tokens de rol AA con variante dark (`text-warning-text` 7.63:1, `text-success-text` 7.56:1, `bg-warning-subtle`); `badge.tsx` ya estaba limpio. `aria-live` lo aportan el Toaster (sonner role status/alert) y los `FormMessage` (aria-invalid+describedby). **Android:** color a nivel token ya AA-verificado (T-227b/RecreColors); los `contentDescription = null` (64) son **decorativos correctos** (icono junto a texto), no violaciones. **FALTA (verificación humana / decisión de diseño, no código):** (1) re-medir contraste sobre el render real en dispositivo en **ambos modos** (no puedo ejecutar la app); (2) pase de **TalkBack/lector** sobre iconos interactivos; (3) **`SignaturePad`** mantiene fondo `#F5F5F5` hardcodeado a propósito (tinta oscura legible sobre "papel" claro en cualquier tema) → necesita un token dedicado `signature-surface` antes de migrarlo, para no romper el contraste de la firma en dark.
...
- [x] **T-246** Bloque 0 — Preparación: baseline verde + lockfiles commiteados; aislar/resolver `next build` roto por `@supabase/ssr` y el test dependiente de CRLF **antes** del Bloque 10. — **Hecho** (PR #27, mergeado). Lockfiles commiteados; baseline verde. *Nota (resuelto 2026-06-16):* el `next build` fallaba en realidad por **2 imports sin usar** (`useState` en `filter-chip.tsx`, `ReactNode` en `subtotal-separator.tsx`) que el lint del build trata como fatales — **no** por `@supabase/ssr` (eso es solo un *warning* de Edge Runtime, no bloquea). Corregidos → `next build` **verde** (30+ rutas + middleware). El "CRLF" era el fichero fuente `registro/actions.ts` guardado en CRLF (no un test; los tests de CSV usan `\r\n` a propósito por RFC 4180) → normalizado a **LF** + prettier limpio. **Baseline limpio para T-256.**
...
- [x] **T-256** Bloque 10 — Web mayor: React 19 + Next 15→16 incremental (params async, ESLint flat config, `next-intl` 4). — **HECHO (4 pasos incrementales, PRs #43/#44/#45 + Next16).** (1) **Next 14→15** + request APIs async (`params`/`searchParams` `Promise`+`await` en 15 páginas; `cookies()` async; `supabase/server.ts createClient()` → **async** con `await cookies()`, sin el escape-hatch `UnsafeUnwrappedCookies`, + `await createClient()` en las 33 capas server). (2) **React 18.3→19** (bump puro: exposición nula a breaking changes, `tsc` 0 sin tocar código). (3) **next-intl 3→4** (config ya compatible; 2 ajustes `?? fallback` por typing más estricto de `t()`). (4) **Next 15→16** (Turbopack por defecto; `tsconfig` `jsx: react-jsx`) + **ESLint 9 flat config** (`eslint.config.mjs` con los flat configs nativos de `eslint-config-next` 16, sin FlatCompat; `next lint`→`eslint src`). Cada paso validado tsc/build/test 65/test/lint/prettier en verde. **DEUDA:** `react-hooks/set-state-in-effect` (regla nueva de react-hooks 7) en **warning**, no error — 4 usos intencionados (guards de montaje + count-up) a revisar caso por caso. **OJO QA (verificación humana, no código):** Next 15/16 cambian defaults de caché (`fetch`/GET handlers ya **no** se cachean por defecto) y Turbopack es el bundler por defecto → clicar la app y verificar que no hay regresión de datos stale, hidratación ni navegación. **Habilita** `useOptimistic` y View Transitions → **desbloquea el shared-element web de T-244**.
