...
- [ ] **T-241** Skeletons de carga, pull-to-refresh (Android) y swipe actions en listados. — **En curso.** **Hecho:** (1) skeletons en los 4 listados de gestión (Máquinas/Locales/Instalaciones/Averías) vía `ListSkeleton`; (2) `Modifier.animateItem()` en los 4 listados de entidad (locales técnico, máquinas, recaudaciones, instalaciones) con el muelle espacial de `RecreMotion` (T-230). **Pull-to-refresh ya existía** en los listados de técnico. **FALTA:** swipe actions (`SwipeToDismissBox`) — pendiente de definir la semántica por lista (¿completar/averiar/archivar?); skeletons en DeudasGestor/DeudasLocal.
