...
- **Respuestas escuetas (Claude, no el usuario).** Prosa al mínimo: sin preámbulo, sin relleno, sin repetir lo obvio. El *entregable* (código, UI, diff, análisis técnico) va completo al 100%; se recorta la cháchara, no el trabajo. Explayarse (trade-offs, diseño) solo si se pide. Avisar al cruzar ~40% de ventana para que el usuario haga `/compact` (no se puede automatizar).
- **Modelo según tarea.** Main loop = Opus (calidad de código + contexto grande, decisión del usuario; no degradar a Sonnet). Abrir un fichero no usa modelo (es E/S; el coste es el resultado en contexto, ya minimizado por RTK/Serena/context-mode). Delegar a un subagente **Explore/Haiku** SOLO la exploración tonta y voluminosa (rastrear N ficheros para hallar algo): lee en su contexto barato y devuelve la conclusión, dejando Opus limpio. Un fichero suelto → leerlo en Opus (delegar no compensa: el arranque del subagente cuesta más). `CLAUDE_CODE_SUBAGENT_MODEL=haiku` ya fija subagentes baratos por defecto.
