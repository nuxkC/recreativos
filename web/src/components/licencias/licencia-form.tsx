...
export function LicenciaForm({ mode, licencia }: LicenciaFormProps) {
  ...
  async function onSubmit(values: LicenciaFormValues) {
    ...
    try {
      ...
      // Si la action redirige (crear con éxito, eliminar) la promesa lanza
      // NEXT_REDIRECT y nunca llegamos aquí. Sólo nos quedan los casos de
      // error y el éxito de "actualizar".
      if (!result.ok) {
