...
Deno.serve(withHandler(async (req: Request) => {
  ...
  // El upsert va con service_role: ya validamos rol operativo arriba y fijamos
  // tecnico_id = userId. La policy de UPDATE del lock exige tecnico_id =
  // auth.uid(), así que con el user client el forzado (`forzar=true`) y la toma
  // de un lock expirado de OTRO técnico fallaban siempre por RLS.
  const { data: row, error: upsertError } = await getServiceClient()
    .from("recaudacion_lock")
    .upsert({
